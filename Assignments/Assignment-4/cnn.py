...
class CNN(object):
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        """
         ...
         :return: Layer object
         """
        object_layer_entity = keras.layers.Conv2D(filters=num_of_filters, kernel_size=kernel_size, padding=padding, strides=strides, activation=activation, name=name, trainable=trainable)
        self.model.add(object_layer_entity)
        return object_layer_entity
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        """
         ...
         :return: Layer object
         """
        object_layer_entity = keras.layers.MaxPooling2D(pool_size=pool_size, padding=padding, strides=strides, name=name)
        self.model.add(object_layer_entity)
        return object_layer_entity
    ...
    def append_flatten_layer(self,name=""):
        """
         ...
         :return: Layer object
         """
        object_layer_entity = keras.layers.Flatten(name=name)
        self.model.add(object_layer_entity)
        return object_layer_entity
    ...
    def set_training_flag(self,layer_numbers=[],layer_names="",trainable_flag=True):
        ...
        if isinstance(layer_numbers, list):
            for i in layer_numbers:
                self.model.get_layer(layer_number=i, layer_name=layer_names[i]).trainable = trainable_flag
        else:
    ...
    def get_weights_without_biases(self,layer_number=None,layer_name=""):
        ...
        if layer_number != None:
            if  layer_number == 0 or len(self.model.layers[layer_number - 1].get_weights()) <= 0 :
                return None
            elif layer_number == -1:
    ...
    def get_biases(self,layer_number=None,layer_name=""):
        ...
        This means that the first layer with activation function is layer zero
         ...
         """
        if layer_number != None:
            if layer_number == 0 or len(self.model.get_layer(index=layer_number - 1).get_weights()) <= 0 :
                return None
            elif layer_number == -1:
    ...
    def remove_last_layer(self):
        ...
        """
        olm_entity = self.model
        self.model = keras.models.Sequential()
        for i in olm_entity._layers[:-1]:
            self.model.add(i)

        return olm_entity.layers[-1]
    ...
    def load_a_model(self,model_name="",model_file_name=""):
        ...
        self.model = keras.models.Sequential()
        if model_name == "VGG16":
            bsm_entity = keras.applications.vgg16.VGG16()
        elif model_name == "VGG19":
            bsm_entity = keras.applications.vgg19.VGG19()
        else:
            bsm_entity = keras.models.load_model(model_file_name)
        for layer in bsm_entity.layers:
